...
prefix_code_and_offset_by_num_bits_in_non_zero_value = [
    (PrefixCode(prefix_code=0b00, num_bits=2), Offset(start=0, num_bits=0)),
    ...
    (PrefixCode(prefix_code=0b1111, num_bits=4), Offset(start=0, num_bits=6)),
    (PrefixCode(prefix_code=0b1111, num_bits=4), Offset(start=0, num_bits=7)),
...
def decode_prefix_code(byte_reader: ByteReader, prefix_tree_root: PrefixCodeNode) -> PrefixCode:
    ...
    while curr and not curr.prefix_code:
        ...
        if bit & 1:
            ...
            print(0, end="")
            curr = curr.zero
    print(f" -> {curr.prefix_code if curr is not None else 'None'}")
    ...
    while block_ind < num_blocks:
        ...
        while num_bytes_in_block < block_size ** 2:
            ...
            try:
                ...
                print(f"failed to decode prefix code after {byte_reader.total_bytes_read} bytes -- {num_bytes_in_block}")
                raise e
